...
class DeviceAdmissionError(RuntimeError):
    def __init__(self, message: str, *, status_code: int = 503, reason: str | None = None) -> None:
...
def _refusal(response: httpx.Response, *, operation: str) -> DeviceAdmissionError:
    ...
    try:
        ...
        if isinstance(detail, dict):
            authority, kind, words = (
                detail.get("authority"), detail.get("kind"), detail.get("detail")
            )
    except ValueError:
        pass
    if words:
    ...
    return DeviceAdmissionError(
        ...
        status_code=response.status_code if response.status_code in {401, 403, 404, 409, 422, 502, 503} else 503,
...
class LocalDeviceOnboardingTarget(BaseModel):
    ...
    operation: Literal["local.device-onboarding-target"] = "local.device-onboarding-target"
    contract_version: Literal["1"] = "1"
    owner_domain_id: str = Field(min_length=1, max_length=128)
    owner_domain_descriptor: OwnerDomainDescriptor
    owner_root_certificate: str = Field(min_length=1, max_length=4096, pattern=r"^-----BEGIN CERTIFICATE-----")
    authority_signing_certificate: str = Field(min_length=1, max_length=4096, pattern=r"^-----BEGIN CERTIFICATE-----")

    @classmethod
    def from_verified(cls, target: VerifiedOwnerDomainOnboardingTarget) -> LocalDeviceOnboardingTarget:
        return cls(
            owner_domain_id=target.owner_domain_id,
            owner_domain_descriptor=target.descriptor,
            owner_root_certificate=target.owner_root_certificate_path.read_text(encoding="ascii"),
            authority_signing_certificate=target.authority_signing_certificate_path.read_text(encoding="ascii"),
        )
...
class LocalEnrollmentDecisionRequest(BaseModel):
    ...
    def to_admin(
        ...
        *,
        ...
        actor = admission_actor(controller_id=controller_id, owner_domain_id=owner_domain_id, approve=True)
...
def admission_actor(*, controller_id: str, owner_domain_id: OwnerDomainId, approve: bool = False) -> ControllerActorRef:
...
def enrollment_query(
    *, controller_id: str, owner_domain_id: OwnerDomainId,
    business_owner_id: BusinessOwnerId, query: EnrollmentProposalQuery,
) -> ControllerEnrollmentQuery:
    return ControllerEnrollmentQuery(
        contract_version="1",
        actor=admission_actor(controller_id=controller_id, owner_domain_id=owner_domain_id),
...
def claim_query(
    *, controller_id: str, owner_domain_id: OwnerDomainId,
    business_owner_id: BusinessOwnerId, query: ClaimQuery,
) -> ControllerClaimQuery:
...
class LocalDeviceRemovalRequest(BaseModel):
    ...
    def to_admin(self, *, device_id: str, owner_id: str, controller_id: str) -> ControllerDeviceRemovalRequest:
        return ControllerDeviceRemovalRequest(
            contract_version="1", request_id=self.request_id, owner_id=owner_id,
            controller_id=controller_id, device_id=device_id, reason="owner-removed",
        )
...
class AdminDeviceAdmissionPort(Protocol):
    async def query_enrollments(self, *, payload: ControllerEnrollmentQuery) -> EnrollmentProposalPage: ...
    async def query_claims(self, *, payload: ControllerClaimQuery) -> ClaimPage: ...
    async def decide(self, *, payload: ControllerEnrollmentDecisionIntent) -> AdmissionDecisionWorkflowResult: ...
    async def remove(
        self, *, payload: ControllerDeviceRemovalRequest, controller_reset_epoch: int,
        authorization_expires_at: datetime, target_device_ref: DeviceRef,
    ) -> DeviceRemovalResult: ...
...
class AdminDeviceAdmissionClient:
    ...
    def _headers(self) -> dict[str, str]:
        if not self._token:
            raise DeviceAdmissionError("Local API Admin service credential is not configured")
    ...
    async def _post(self, path: str, payload: BaseModel, model):
        try:
            response = await self._client.post(
                ...
                headers=self._headers(), json=payload.model_dump(mode="json"), timeout=self._timeout,
            )
        except (httpx.TimeoutException, httpx.NetworkError, httpx.RemoteProtocolError) as exc:
            raise DeviceAdmissionError("Admin Device admission control plane is unavailable") from exc
        if response.status_code != 200:
            raise _refusal(response, operation="admission query")
        try:
            return model.model_validate_json(response.content)
        except (ValueError, TypeError, ValidationError) as exc:
            raise DeviceAdmissionError("Admin Device admission response violated its contract") from exc

    async def query_enrollments(self, *, payload: ControllerEnrollmentQuery) -> EnrollmentProposalPage:
        page = await self._post("admission/enrollment-queries", payload, EnrollmentProposalPage)
        if page.owner_domain_id != payload.query.owner_domain_id:
            raise DeviceAdmissionError("Admin enrollment query returned another Owner Domain", status_code=502)
        return page

    async def query_claims(self, *, payload: ControllerClaimQuery) -> ClaimPage:
        page = await self._post("admission/claim-queries", payload, ClaimPage)
        if page.owner_domain_id != payload.query.owner_domain_id:
            raise DeviceAdmissionError("Admin Claim query returned another Owner Domain", status_code=502)
        return page
    ...
    async def decide(self, *, payload: ControllerEnrollmentDecisionIntent) -> AdmissionDecisionWorkflowResult:
        ...
        try:
            result = AdmissionDecisionWorkflowResult.model_validate_json(response.content)
        except (ValueError, TypeError, ValidationError) as exc:
            raise DeviceAdmissionError("Admin Device Decision response violated its contract") from exc
        if result.request_id != payload.request_id:
            raise DeviceAdmissionError("Admin Device Decision returned another request", status_code=409)
        return result
    ...
    async def remove(
        ...
        try:
            reply = await asyncio.wait_for(
                self._remove_over_workflow_socket(LifecycleRemovalCall(
                    payload=payload,
                    authorization_context=RemovalOwnerAuthorizationContext(
                        ...
                        owner_authorization_context=OwnerAuthorizationContext(
                            ...
                            actor=ActorRef(
                                principal_id=payload.controller_id, principal_type="controller",
                                owner_domain_id=owner_domain_id,
                                granted_scopes=("device.read", "device.claim.revoke"),
                                authentication_strength="software",
                            ),
                            authorized_owner_domain_id=owner_domain_id,
                            scopes=("device.read", "device.claim.revoke"), intent_id=intent_id,
                            target_device_ref=target_device_ref, issued_at=datetime.now(UTC),
        ...
        if reply.problem is not None:
            raise DeviceAdmissionError(reply.problem.detail, status_code=reply.problem.status_code)
        if reply.result is None or reply.result.request_id != payload.request_id:
            raise DeviceAdmissionError("Lifecycle Workflow returned another request", status_code=409)
        return reply.result

    async def _remove_over_workflow_socket(self, call: LifecycleRemovalCall) -> LifecycleWorkflowReply:
        reader, writer = await asyncio.open_unix_connection(path=str(self._workflow_socket_path))
        try:
...
def device_removal_progress(
    *, owner_id: str, device_id: str, result: DeviceRemovalResult
    ...
    if hub is not None and (
        ...
        raise DeviceAdmissionError("Admin Device removal did not confirm the requested device", status_code=502)
    required = {
    ...
    if {condition.name for condition in result.conditions} != required:
        raise DeviceAdmissionError("Admin Device removal returned an incomplete condition projection", status_code=502)
    return LocalDeviceRemovalProgress(
        request_id=result.request_id, device_id=device_id, owner_id=owner_id,
        intent_id=result.intent_id,
        outcome="done" if result.outcome == "completed" else "unfinished" if result.outcome == "accepted" else "refused",
