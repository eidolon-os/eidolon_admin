...
What goes here vs not:
    - Tenants: admin-only concept, no sub-project equivalent → local SQLite.
    ...
    - User metadata also does not live here; admin persists it in local
      SQLite so user CRUD is not coupled to NATS KV.
...
Size limits:
    Each value is a small JSON object (agent metadata, binding pointer).
...
# 4 KB caps flat binding / metadata JSON. If we ever need to store more
# here, prefer adding a new bucket over inflating these.
