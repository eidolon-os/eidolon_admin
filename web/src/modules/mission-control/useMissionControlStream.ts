...
/** As much of an Owner as anything still publishes: the id you asked for. */
...
export function useMissionControlStream(opts: MissionControlMode = {}) {
  ...
  // The Owner this cockpit is about, if anyone has said. Admin used to list
  // every Owner from its own database; Data owns them now and publishes no
  // list, so there is nobody to enumerate and nobody to default to.
  ...
  async function loadOwners() {
    // Nothing to load. No authority publishes an Owner roster over HTTP, so
    // this used to pick the first row of a list that came from Admin's own
    // database — and on a Host with two Owners it would have shown one
    // person's devices under the other's name. The snapshot reports the same
    // gap in source_status; the cockpit shows whichever Owner it was given.
    owners.value = ownerId.value
      ? [{ owner_id: ownerId.value, display_name: '' }]
      : []
  }
