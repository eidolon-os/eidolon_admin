...
<template>
  <div class="lane perm-ledger">
    <span class="pl-cap"><i class="led mag" />权限台账 · PERMISSION<em v-if="items.length" class="pl-n">{{ items.length }}</em></span>
    <ul v-if="items.length" class="pl-list">
    ...
    <p v-else class="pl-idle">无敏感调用</p>
  </div>
</template>

<style scoped>
.perm-ledger { display: flex; flex-direction: column; gap: 6px; padding: 8px 14px; border: 1px solid rgba(255, 46, 136, 0.28); background: var(--cy-panel); }
...
.pl-n { font-style: normal; color: var(--cy-mag); }
.pl-idle { margin: 0; font: 400 11px/1 var(--cy-sans); color: var(--cy-txt-dim); }
