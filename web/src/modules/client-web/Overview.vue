...
// Mirror of channel/Overview — process-only service status dashboard built
// from supervisor process info + the optional HTTP probe (Next dev server
// answers on :3000 so we get real reachability data here too).
...
async function load() {
  ...
  try {
    ...
    status.value = o.services.find((s) => s.id === 'client-web') || null
    try {
      program.value = await getProgram('client-web:client-web')
    } catch {
      program.value = null
    }
  } finally {
...
<template>
  <div class="page">
    <div class="topbar">
      <div class="title-row">
        <h2 class="title">Client Web (Next.js)</h2>
    ...
    >
      <template #title>
        client-web.conf 尚未启用。进入 Supervisor 页面打开开关后这里会显示进程详情。
      </template>
    </el-alert>
      ...
      <div class="stat-card">
        ...
        <div class="stat-val small">
          <template v-if="httpProbe?.ok">
            <span class="ok">{{ httpProbe.status_code }} ({{ httpProbe.latency_ms }}ms)</span>
          </template>
          <template v-else-if="httpProbe?.configured">
            <span class="bad">{{ httpProbe.error || `HTTP ${httpProbe.status_code}` }}</span>
          </template>
    ...
    <el-card v-if="program" style="margin-top: 16px">
      ...
      <p class="hint">
        Next.js dev 把所有输出都打到 stdout（不区分 info/error）。stderr 通常只在崩溃栈时有内容。
      </p>
      <el-space>
    ...
    <el-card style="margin-top: 16px">
      ...
      <p class="mono link">
        浏览器打开 <a href="http://127.0.0.1:3000" target="_blank">http://127.0.0.1:3000</a>
      </p>
      <p class="hint">
        客户端依赖 hub (token 接口) 和 livekit (信令)，需要都在线。
      </p>
    </el-card>

    <LogViewer
      v-model:open="logOpen"
      :title="`client-web :: ${logStream}`"
      :url="tailLogUrl('client-web:client-web', logStream)"
    />
  </div>
</template>
